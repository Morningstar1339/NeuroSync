...
import { Supplement, Exclusion, TimeWindowParams, DosageLimitParams, StudyProtocol, getSupplementExclusions, addExclusion, updateExclusion, deleteExclusion, getSupplementStudyProtocols, addStudyProtocol, deleteStudyProtocol } from '@/database/supplements';
import { Ionicons } from '@expo/vector-icons';
import { useThemeColor } from '@/hooks/use-theme-color';
import { useRouter, useLocalSearchParams } from 'expo-router';
import { openDatabase } from '@/database/database';
import { scheduleSupplementReminders, scheduleBasedStudyNotifications } from '@/services/reminder-scheduler';
import { cancelNotificationsByRelatedId } from '@/database/notifications';
...
export default function EditSupplementScreen() {
  ...
  const loadExclusions = async () => {
    try {
      const supplementExclusions = await getSupplementExclusions(originalSupplement.id);
  ...
  const loadStudyProtocols = async () => {
    try {
      const protocols = await getSupplementStudyProtocols(originalSupplement.id);
  ...
  const getTestDuration = (testType: 'reflexes' | 'memory' | 'judgment'): number => {
    switch (testType) {
      case 'reflexes': return 10/60; // 10 seconds in minutes
      case 'memory': return 1; // 60 seconds in minutes
      case 'judgment': return 0.5; // 30 seconds in minutes
      default: return 1;
    }
  };
  ...
  const getTestDisplayName = (testType: 'reflexes' | 'memory' | 'judgment'): string => {
    switch (testType) {
      case 'reflexes': return 'Reflexes (10s)';
      case 'memory': return 'Memory (60s)';
      case 'judgment': return 'Judgment (30s)';
